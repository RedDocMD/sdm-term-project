...
# ╔═╡ 6a568884-437e-11ec-33c2-8f959528e426
begin
	using Plots
	using PlotlyJS
	using StatPlots
...
# ╔═╡ d0eb0c99-eb81-4ac8-9042-56227414a5c9
begin
	struct RunTime
		min::Int
		sec::Int
	...
	toMin(rt) = Float32(rt.min) + Float32(rt.sec) / Float32(60)
end

# ╔═╡ f4e949ab-dd99-4cd8-a31c-e3a956c47ef4
begin
	# The order is same as that of my notes
	# First Julia then Python
	times = [
		RunTime(2, 23), RunTime(2, 34), RunTime(4, 10), RunTime(4, 59), RunTime(4, 48),
		RunTime(18, 2), RunTime(18, 58), RunTime(32, 15), RunTime(34, 22), RunTime(37, 27)
	]
	timeInMin = map(toMin, times)
end

# ╔═╡ 79140879-906c-40c7-95f3-9b08e1d8683b
begin
	nam = repeat("G" .* string.(1:5), outer = 2)
	lang = repeat(["Julia", "Python"], inner = 5)

	p1 = StatPlots.groupedbar(nam, timeInMin, group = lang, ylabel = "Time (min)", 
        title = "Julia vs Python (Standard SGD)", minorticks = 2, minorgrid = true)
end

# ╔═╡ 850dec34-f1c8-40e6-8cc2-e4fe84fcc9fd
Plots.savefig(p1, "plots/perf_sgd.eps")
